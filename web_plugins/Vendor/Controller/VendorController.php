...
class VendorController extends VendorAppController {
    ...
    public function admin_import(){
        ...
        if(!empty($this->request->data)){
            ...
            if($path_parts['extension'] == 'xls' || $path_parts['extension'] == 'xlsx'){
                ...
                $Reader = new SpreadsheetReader($this->request->data['Import']['pricelist']['tmp_name'], $this->request->data['Import']['pricelist']['name'], 'application/xlsx');
                ...
                foreach($data as $key => $_data){
                    if(!empty($_data[($vendor['data']['col_code']-1)])){
                        ...
                        $save = array(
                            'type' => 'vendor',
                            "extra_1" => $vendor['id'],
                            "extra_3" => (!empty($vendor['data']['col_base']) || !empty($vendor['data']['col_prodcode']) ? '1' : '0'),
                        ...
                        if(empty($save['extra_6'])) continue;
                        ...
                        if(!empty($vendor['data']['col_base']) || !empty($vendor['data']['col_prodcode'])){
                            $save['data']['pricelist'] = $_data;
                        }
    ...
    public function admin_insert($vendor_id){
        set_time_limit(0);
        ini_set('memory_limit', '-1');
        
        $this->set('page_title', ___('Vendors') . ' :: ' . ___('Insert'));

        $vendor = Configure::read("Obj.vendors.{$vendor_id}.ObjItemList");

        $rows = $this->ExtraData->find('all', array('conditions' => array('ExtraData.extra_1' => $vendor['id'], 'ExtraData.type' => 'vendor', 'ExtraData.extra_3' => '1'), 'limit' => '100'));
        foreach($rows as $row){

    
            $_data = $row['ExtraData']['data']['pricelist'];

            if(!empty($vendor['data']['col_prodcode'])){
                $base_id = null;
                
                if(!empty($vendor['data']['col_base'])) foreach($vendor['data']['bases'] as $bases_base_id => $bases){
                    if(empty($bases)) continue;
                    $bases = explode(',', $bases);
                    if(in_array(trim($_data[($vendor['data']['col_base']-1)]), $bases)){
                        $base_id = $bases_base_id;
                        break;
                    }
                }
                if(!empty($base_id) || 1==1){
                    Configure::write('Config.tid', 'catalog');
                    $id = $this->ObjItemList->field('id', array('ObjItemList.code' => trim($_data[($vendor['data']['col_prodcode']-1)])));
                    if(empty($id)){
                        $id = $this->ObjItemList->field('id', array('ObjItemList.title' => trim($_data[($vendor['data']['col_title']-1)])));
                    }
                    if(empty($id)){
                        
                        $brand_id = null;
                        if(!empty($vendor['data']['col_brand'])){
                            $brand_id = $this->ObjItemList->field('id', array('tid' => 'manufacturer', 'ObjItemList.title' => trim($_data[($vendor['data']['col_brand']-1)])));
                            if(empty($brand_id)){
                                $this->ObjItemList->create();
                                $this->ObjItemList->save(array('tid' => 'manufacturer', 'title' => trim($_data[($vendor['data']['col_brand']-1)]), 'status' => '1'));
                                $brand_id = $this->ObjItemList->getLastInsertId();
                            }
                        }

                        $this->ObjItemList->create();
                        $this->ObjItemList->save(array('ObjItemList' => array(
                            'title' => trim($_data[($vendor['data']['col_title']-1)]),
                            'alias' => ws_alias(trim($_data[($vendor['data']['col_title']-1)])),
                            'code' => trim($_data[($vendor['data']['col_prodcode']-1)]),
                            //'short_body' => trim($_data[($vendor['data']['col_prodcode']-1)]),
                            'qnt' => 1,
                            'base_id' => $base_id,
                            'extra_2' => $brand_id,
                            'data' => array(
                                'wrnt' => preg_replace("/[^0-9]/", '', $_data[($vendor['data']['col_wrnt']-1)]) * 12,
                            )
                        )));

                        $item_id = $this->ObjItemList->getLastInsertId();

                        $this->ObjItemList->ObjOptRelation->update_insert(array(
                            'rel_id' => $vendor['id'], 
                            'value' => trim($_data[($vendor['data']['col_code']-1)])
                        ), array(
                            'model' => 'ObjItemList', 
                            'type' => 'vendor_code', 
                            'foreign_key' => $item_id
        ...
        if(empty($rows)){
            $this->Session->setFlash(___('Pricelist inserted successfull.'), 'flash');
    ...
    public function admin_update($upd_item_id = null){
        ...
        if(empty($upd_item_id)){
            if(empty($_GET['continue'])){
                file_put_contents(LOGS . DS . 'vendor_update.log', '');
            } else {
                $exclude = file_get_contents(LOGS . DS . 'vendor_update.log');
            }
        }

        $conds = array('ObjOptRelation.type' => 'vendor_code', "ObjOptRelation.value != ''");
        if(!empty($upd_item_id)) $conds = am($conds, array('ObjOptRelation.foreign_key' => $upd_item_id));
        if(!empty($exclude)){
            $conds = am($conds, array("ObjOptRelation.foreign_key NOT IN (".rtrim($exclude, ',').")"));
        }
        $rows = $this->ObjItemList->ObjOptRelation->find('all', array('conditions' => $conds, 'order' => array('foreign_key' => 'asc'), 'limit' => '100'));
        $_rows = array();
        foreach($rows as $row){
            $_rows[$row['ObjOptRelation']['foreign_key']][] = $row;
        }
        $rows = $_rows;
        //_pr($conds);
        //_pr($rows);
        //exit;
        foreach($rows as $item_id => $_rows){
            
            if(empty($upd_item_id)) file_put_contents(LOGS . DS . 'vendor_update.log', $item_id . ',', FILE_APPEND);
            ...
            if(!empty($rel_id)){
                ...
                $item['base_id'] = $this->ObjItemList->field('base_id', array('ObjItemList.id' => $rel_id));
            } else {
                $item['base_id'] = $this->ObjItemList->field('base_id', array('ObjItemList.id' => $item_id));
            }
            $item['coeficient'] = $this->ObjItemList->ObjOptRelation->field('value', array('type' => 'vendor_percent', 'rel_id' => '1', 'foreign_key' => $item_id));
            $item['coeficient_fix'] = $this->ObjItemList->ObjOptRelation->field('value', array('type' => 'vendor_fix', 'rel_id' => '1', 'foreign_key' => $item_id));
            if(empty($bases[$item['base_id']])){
                $bases[$item['base_id']] = array_reverse(Set::extract('/ObjItemTree/id', $this->ObjItemTree->getPath($item['base_id'], array('id'))));
            }

            foreach($_rows as $row){
                $_price = $this->ExtraData->find('first', array('conditions' => array('ExtraData.type' => 'vendor', 'ExtraData.extra_5' => $row['ObjOptRelation']['value'])));

                if(empty($_price['ExtraData']['extra_6'])) continue;
            
                $vendor_id = $_price['ExtraData']['extra_1'];
                $vendor = $vendors[$vendor_id]['ObjItemList']['data'];

                $price = array(
                    'vendor_id' => $vendor_id, 
                    'price' => $_price['ExtraData']['extra_6'],
                    'extra_4' => $_price['ExtraData']['extra_4'],
                    'currency' => $_price['ExtraData']['extra_7'],
                    'price_orig' => $_price['ExtraData']['extra_6'],
                    'currency_orig' => $_price['ExtraData']['extra_7'],
                    'c_price_orig' => $_price['ExtraData']['extra_6'],
                    'c_currency_orig' => $_price['ExtraData']['extra_7'],
                );
        

                if((!empty($vendor['col_price_calc']) || !empty($_price['ExtraData']['extra_4']))){
                    if(!empty($_price['ExtraData']['data']['price_calc'])){
                        if(empty($_price['ExtraData']['data']['currency_calc'])) $_price['ExtraData']['data']['currency_calc'] = $_price['ExtraData']['extra_7'];
                        $price['price'] = $_price['ExtraData']['data']['price_calc'];
                        $price['currency'] = $_price['ExtraData']['data']['currency_calc'];
                    } else {
                        //continue;
                    }
                }
                ...
                $this->ExtraData->updateAll(array(
                    ...
                    "ExtraData.data" => sqls(json_encode(am($_price['ExtraData']['data'], $price)), true),
                ), array("ExtraData.id" => $_price['ExtraData']['id']));
            ...
            $items_price = array();
            foreach($prices as $vendor_id => $price){
    ...
    public function __admin_update($upd_item_id = null){
        set_time_limit(0);
        ini_set('memory_limit', '-1');
