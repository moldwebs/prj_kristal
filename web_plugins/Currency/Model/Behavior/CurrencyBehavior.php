...
class CurrencyBehavior extends ModelBehavior {
   ...
   public function beforeFind(Model $model, $query) {
        ...
        if($model->__tid != 'catalog') return $query;
            ...
            foreach($results as $key => $result){
                ...
                if(!empty($result['ModCurrency'])){
                    ...
                    foreach($this->settings[$model->alias]['currencies'] as $currency){
                        ...
                        $currency_html_vals[$currency['currency']] = str_replace($value_dec_trim, '', number_format((($result[$model->alias]['price']*$result['ModCurrency']['value'])/$currency['value']), $value_dec, '.', ' ')) . ' ' . $currency['title'];
                    }
