<?php
class PromoItemController extends PromoAppController {
    ...
    public function admin_edit_promo($id = null){
    ...
    public function get_list($base_id = null){
        ...
        if($base_id > 0){
            ...
            $promo = $this->ObjItemTree->find('first', array('conditions' => array('ObjItemTree.id' => $base_id)));
        } else {
            $promo = $this->ObjItemTree->find('first', array('conditions' => array('ObjItemTree.code' => $base_id, 'ObjItemTree.tid' => Configure::read('Config.tid'))));
            
            $conditions = am($conditions, array('ObjItemTree.parent_id' => $promo['ObjItemTree']['id']));
        }
        
        $items = $this->ObjItemTree->find('all', array('conditions' => $conditions, 'order' => 'ObjItemTree.lft'));
        
        return array('items' => $items, 'promo' => $promo);
    }
}
