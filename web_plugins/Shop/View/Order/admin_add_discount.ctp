...
<?php else:?>
    <div style="width: 400px;">
        ...
        <form class="no-enter" id="add_discount">
        <div class="n1 cl">
        ...
        <script>
            $('#BtnSubmit').click(function(){
                if($('#add_discount [name="data[title]"]').val() != '' && parseFloat($('#add_discount [name="data[value]"]').val()) > 0){
                    $.post('/admin/shop/order/add_discount?currency=<?php e($_GET['currency'])?>', $('#add_discount').serialize(), function(data){$('#order_discount').append(data);ajx_win_close();order_calc();});
                }
            });
        </script>
        </form>
