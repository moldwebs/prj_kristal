 ...
 */
	protected function _deleteDependent($id, $cascade) {
		...
		foreach (array_merge($this->hasMany, $this->hasOne) as $assoc => $data) {
			...
			if (isset($data['exclusive']) && $data['exclusive'] || 1==1) {
 ...
 */
	public function deleteAll($conditions, $cascade = false, $callbacks = false) {
        ...
        //file_put_contents(LOGS . DS . 'db_ops_'.date("Ymd").'.log', date("Y-m-d H:i:s")."-DeleteAll ".Configure::read('TMP.lquery')."-{$_SERVER['REQUEST_URI']}-\r\n" . "\r\n", FILE_APPEND);
