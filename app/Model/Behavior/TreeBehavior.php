	...
	public function beforeDelete(Model $Model, $cascade = true) {
		...
		if ($data) {
    		$_data = $Model->find('first', array(
    			'conditions' => array($Model->escapeField('parent_id') => $Model->id),
    			'fields' => array($Model->escapeField($Model->primaryKey), $Model->escapeField($Model->primaryKey)),
    			'recursive' => -1));
 ...
 */
	public function afterDelete(Model $Model) {
		...
		if ($diff > 2) {
			...
			$scope[]["{$Model->alias}.tid"] = Configure::read('Config.tid');
	...
	public function beforeSave(Model $Model) {
		...
		if (!$Model->id || !$Model->exists()) {
			if (array_key_exists($parent, $Model->data[$Model->alias]) && $Model->data[$Model->alias][$parent]) {
                ...
                if(!empty($Model->data[$Model->alias]['tid'])){
                    $find_cond['tid'] = $Model->data[$Model->alias]['tid'];
                    $find_cond['uid'] = cms_uid($Model->alias, Configure::read('Config.tid'));
                }
                $parentNode = $Model->find('first', $find_cond);
				if (!$parentNode) {
					return false;
				}
			...
			if (!$Model->data[$Model->alias][$parent]) {
				...
				$values = $Model->find('first', $find_cond);
 ...
 */
	...
	public function childrens(Model $Model, $id = null, $field = null, $include_parent = true) {
		if (is_array($id)) {
			extract(array_merge(array('id' => null), $id));
		}
		...
		if (!$id) {
			...
			if (empty($result) || !isset($result[0])) {
                return ($include_parent && $field == 'id' ? array($id) : array());
			}

            if($include_parent){
                $conditions = array($scope, array('OR' => array($Model->escapeField() => $id, array($Model->escapeField($right) . ' <' => $result[0][$right], $Model->escapeField($left) . ' >' => $result[0][$left]))));
            } else {
                $conditions = array($scope, $Model->escapeField($right) . ' <' => $result[0][$right], $Model->escapeField($left) . ' >' => $result[0][$left]);
            }
			
		}
        $return = ($field ? Set::extract("/{$Model->alias}/{$field}", $Model->find('all', compact('conditions', 'fields', 'order', 'limit', 'page', 'recursive'))) : $Model->find('all', compact('conditions', 'fields', 'order', 'limit', 'page', 'recursive')));
		if($include_parent && empty($return) && $field == 'id') $return = array($id);
        return $return;
	}
 ...
 */
	public function getPath(Model $Model, $id = null, $fields = null, $recursive = null, $exclude = null) {
		if (is_array($id)) {
			extract(array_merge(array('id' => null), $id));
		}
		$overrideRecursive = $recursive;
		...
		if(!empty($exclude)) $conditions["{$Model->alias}.id <>"] = $exclude;
 ...
 */
	protected function _getMax(Model $Model, $scope, $right, $recursive = -1, $created = false) {
		...
		list($edge) = array_values($Model->find('first', $find_cond));
 ...
 */
	protected function _sync(Model $Model, $shift, $dir = '+', $conditions = array(), $created = false, $field = 'both') {
        ...
        if($scope == '1 = 1'){
            $scope = "`{$Model->alias}`.`tid` = '" . Configure::read('Config.tid') . "' AND `{$Model->alias}`.`uid` = '" . cms_uid($Model->alias, Configure::read('Config.tid')) . "'";
        }
		$Model->recursive = $recursive;
