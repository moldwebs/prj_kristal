 ...
 */
	...
	public function __link($title, $url = array(), $options = array()) {
		$options = array_merge(array('model' => null, 'escape' => true), $options);
		$model = $options['model'];
		unset($options['model']);
        ...
        if(is_array($url)){
            if(!empty($url['?']['is_ajax_simple'])) unset($url['?']['is_ajax_simple']);
            if(strpos($_SERVER['REQUEST_URI'], "/{$url[0]}") === false) unset($url[0]);
        }
        
		return $this->url($url, false, $model);
	}
	...
	public function numbers($options = array()) {
		...
		if ($modulus && $params['pageCount'] > $modulus) {
			...
			for ($i = $start; $i < $params['page']; $i++) {
                ...
                $_out['pages'][] = array('number' => $i, 'url' => $this->__link($i, array('page' => $i), $options));
			}
			...
			if ($currentTag) {
				$out .= $this->Html->tag($tag, $this->Html->tag($currentTag, $this->link($params['page'], array('page' => $params['page']), array('class' => $currentClass))), array('class' => $currentClass));
			} else {
				$out .= $this->Html->tag($tag, $this->link($params['page'], array('page' => $params['page']), array('class' => $currentClass)), array('class' => $currentClass));
			}
            
            $_out['pages'][] = array('number' => $params['page'], 'url' => $this->__link($params['page'], array('page' => $params['page']), array('class' => $currentClass)), 'active' => true);
			...
			if ($end != $params['page']) {
                ...
                $_out['pages'][] = array('number' => $end, 'url' => $this->__link($i, array('page' => $end), $options));
			}
			...
			for ($i = 1; $i <= $params['pageCount']; $i++) {
				if ($i == $params['page']) {
					if ($currentTag) {
						$out .= $this->Html->tag($tag, $this->Html->tag($currentTag, ' ' . $this->link($i, array('page' => $i)) . ' ', array('class' => $currentClass)), array('class' => $currentClass));
					} else {
						$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i), array('class' => $currentClass)), array('class' => $currentClass));
					}
                    $_out['pages'][] = array('number' => $i, 'url' => $this->__link($i, array('page' => $i), array('class' => $currentClass)), 'active' => true);
				} else {
					$out .= $this->Html->tag($tag, $this->link($i, array('page' => $i), $options), compact('class'));
                    $_out['pages'][] = array('number' => $i, 'url' => $this->__link($i, array('page' => $i), array('class' => $currentClass)));
				}
				if ($i != $params['pageCount']) {
					$out .= $separator;
				}
        ...
        if(!empty($_out)){
            if($params['page'] > 1) $_out['first'] = array('number' => 1, 'url' => $this->__link('', array('page' => 1)));
            if($params['page'] < $params['pageCount']) $_out['last'] = array('number' => $params['pageCount'], 'url' => $this->__link('', array('page' => $params['pageCount'])));
            
            if($params['page'] > 1) $_out['prev'] = array('number' => ($params['page'] - 1), 'url' => $this->__link('', array('page' => ($params['page'] - 1))));
            if($params['page'] < $params['pageCount']) $_out['next'] = array('number' => ($params['page'] + 1), 'url' => $this->__link('', array('page' => ($params['page'] + 1))));
        }

		return ($options['array'] !== true ? $out : $_out);
	}
