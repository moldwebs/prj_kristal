 ...
 */
	public static function items($items = null) {
        ...
        if (!empty($items)) {
          ...
          $role_perms = Configure::read('role_perms');
          if(!empty($role_perms)){
              foreach($return as $key => $val){
                if(!empty($val['children'])){
                    foreach($val['children'] as $_key => $_val){
                        if(empty($role_perms[$_val['url']['plugin']][$_val['url']['controller']][$_val['url']['action']])){
                            unset($return[$key]['children'][$_key]);
                            continue;
                        }
                        if(!empty($_val['buttons'])){
                            foreach($_val['buttons'] as $__key => $__val){
                                if(empty($role_perms[$__val['url']['plugin']][$__val['url']['controller']][$__val['url']['action']])) unset($return[$key]['children'][$_key]['buttons'][$__key]);
                            }
                        }
                    }
                }
              ...
              foreach($return as $key => $val){
                if(empty($val['children']) && is_array($val['url'])) unset($return[$key]);
              }
          }
