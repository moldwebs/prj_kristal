...
*/
		...
		fileShow: function()
		{
			...
			var callback = $.proxy(function()
			{
				...
				if (this.opts.fileGetJson)
				{
					$.getJSON(this.opts.fileGetJson, $.proxy(function(data)
					{
						...
						$.each(data, $.proxy(function(key, val)
						{
							...
							var img = $('<div><img src="' + val.thumb + '" class="redactorfolder redactorfolder' + folderkey + '" rel="' + val.image + '" title="' + thumbtitle + '" /><br>' + thumbtitle + '</div>');
							$('#redactor_file_box').append(img);
							$(img).click($.proxy(this.fileThumbClick, this));
				...
				if (this.opts.fileUpload || this.opts.s3)
				{
					// dragupload
				...
				$('#redactor_upload_btn').click($.proxy(this.fileCallbackLink, this));

				if (!this.opts.fileUpload && !this.opts.fileGetJson)
				{
					setTimeout(function()
					{
			...
			this.modalInit(this.opts.curLang.file, this.opts.modal_file, 610, callback);
		...
		imageEdit: function(image)
		{
			...
			var callback = $.proxy(function()
			{
				...
				$('#redactor_file_width').val($el.css('width').replace('px', ''));
				//$('#redactor_file_height').val($el.css('height').replace('px', ''));

				if($el.css('padding-left').replace('px', '') > 0) $('#redactor_file_pdl').val($el.css('padding-left').replace('px', ''));
				if($el.css('padding-right').replace('px', '') > 0) $('#redactor_file_pdr').val($el.css('padding-right').replace('px', ''));
				if($el.css('padding-top').replace('px', '') > 0) $('#redactor_file_pdt').val($el.css('padding-top').replace('px', ''));
				if($el.css('padding-bottom').replace('px', '') > 0) $('#redactor_file_pdb').val($el.css('padding-bottom').replace('px', ''));
		...
		imageSave: function(el)
		{
			...
			$el.css('width', $('#redactor_file_width').val());
			//$el.css('height', $('#redactor_file_height').val());

			if($('#redactor_file_pdl').val() > 0) $el.css('padding-left', $('#redactor_file_pdl').val());
			if($('#redactor_file_pdr').val() > 0) $el.css('padding-right', $('#redactor_file_pdr').val());
			if($('#redactor_file_pdt').val() > 0) $el.css('padding-top', $('#redactor_file_pdt').val());
			if($('#redactor_file_pdb').val() > 0) $el.css('padding-bottom', $('#redactor_file_pdb').val());
		...
		fileThumbClick: function(e)
		{
			var img = '<a href="' + $(e.target).attr('rel') + '">' + ($('#redactor_filename').val() !== '' ? $('#redactor_filename').val() : $(e.target).attr('title')) + '</a>';
		...
		modalTemplatesInit: function()
		{
			$.extend( this.opts,
			{
				...
				+ '<section>'
					...
					+ '<br>'
                    + '<select style="width: 60%;" id="redactor_form_image_align">'
						+ '<option value="none">' + this.opts.curLang.image_position + '</option>'
						+ '<option value="left">' + this.opts.curLang.left + '</option>'
						+ '<option value="right">' + this.opts.curLang.right + '</option>'
					+ '</select>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;'
                    + '<input style="width: 28%;" id="redactor_file_width" placeholder="width"> px'
					+ '<br>'
					+ '<label>' + this.opts.curLang.indent + '</label>'
                    + '<input style="width: 21%;" id="redactor_file_pdl" placeholder="left"> &nbsp;'
                    + '<input style="width: 21%;" id="redactor_file_pdr" placeholder="right"> &nbsp;'
                    + '<input style="width: 21%;" id="redactor_file_pdt" placeholder="top"> &nbsp;'
                    + '<input style="width: 21%;" id="redactor_file_pdb" placeholder="bottom"> &nbsp;'
				+ '</section>'
				+ '<footer>'
